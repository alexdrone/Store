...
public let sharedLogger = Logger(label: "io.store")

@dynamicMemberLookup
public final class Query {
  private var segments: [String] = []
  private let signpostDiff: SignpostDiff
  
  init(signpostDiff: SignpostDiff) {
    self.signpostDiff = signpostDiff
  }
  ...
  /// Returns `true` if the key path was added in this signpost store update, `false` otherwise.
  public func toPropertyDiff() -> PropertyDiff {
    signpostDiff.diffs[self.toKeyPath()] ?? .unspecified
  }
}

/// A collection of changes associated to a signpost store update.
@frozen public struct SignpostDiff {
  ...
  /// Reference to the signpost update that cause this change.
  ...
  public func query(_ keyPath: (Query) -> Query) -> PropertyDiff {
    keyPath(Query(signpostDiff: self)).toPropertyDiff()
  }
  
  public init(signpost: String, diffs: [FlatEncoding.KeyPath: PropertyDiff]) {
...
public enum PropertyDiff {
  ...
  /// Returns `true` if the key path was added in this signpost update, `false` otherwise.
  ...
  /// Returns `true` if the key path was removed in this signpost update, `false` otherwise.
  ...
  /// Returns `true` if the key path was changed in this signpost update, `false` otherwise.
  ...
  /// Returns the new value if the property was just added in this signpost update, `nil` otherwise.
  ...
  /// Returns the tuple (`old`, `new`) if the value of the property changed in this signpost update.
...
///   tokens/1: "bar"
/// } ```
public func flatten(encodedObject: EncodedDictionary) -> FlatEncoding.Dictionary {
  var result: FlatEncoding.Dictionary = [:]
  FlatEncoding.flatten(path: "", node: .dictionary(encodedObject), result: &result)
  return result
}
...
public enum FlatEncoding {
  ...
  /// - note: See `flatten(encodedObject:)`.
    ...
    func process(path: String, value: Any) {
      if let dictionary = value as? [String: Any] {
        ...
        guard let keyPath = KeyPath(path) else {
          sharedLogger.error("Malformed FlatEncoding keypath: \(path).")
          return
        }
