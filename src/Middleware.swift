...
open class Middleware {
  ...
  open func willDispatch(transaction: String, action: ActionType, in store: AnyStore) { }

  /** An action just got dispatched. */
  open func didDispatch(transaction: String, action: ActionType, in store: AnyStore) { }
}

extension Array where Element: Middleware {

  /** Propagates the 'willDispatch' callback to all of the elements. */
  func willDispatch(transaction: String, action: ActionType, in store: AnyStore) {
  ...
  func didDispatch(transaction: String, action: ActionType, in store: AnyStore) {
    for middleware in self {
