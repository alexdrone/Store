 ...
 */
public func wrap<T>(_ object: T,
                    context: Any? = nil,
                    dateFormatter: DateFormatter? = nil) throws -> WrappedDictionary {
  return try Wrapper(context: context, dateFormatter: dateFormatter)
      .wrap(object: object, enableCustomizedWrapping: true)
}
...
public func wrap<T>(_ object: T,
  ...
  return try Wrapper(context: context, dateFormatter: dateFormatter)
      .wrap(object: object, writingOptions: writingOptions ?? [])
}
 ...
 *  See the documentation for the dictionary-based `wrap()` function for more information
 */
public func wrap<T>(_ objects: [T],
                    context: Any? = nil,
                    dateFormatter: DateFormatter? = nil) throws -> [WrappedDictionary] {
...
public protocol WrapCustomizable {
   ...
   */
  func wrap(propertyNamed propertyName: String,
            originalValue: Any,
            context: Any?,
...
public extension WrapCustomizable {
  ...
  func wrap(propertyNamed propertyName: String,
            ...
            dateFormatter: DateFormatter?) throws -> Any? {
    return try Wrapper(context: context, dateFormatter: dateFormatter)
        .wrap(value: originalValue, propertyName: propertyName)
  }
}
...
extension NSArray: WrapCustomizable {
  ...
  extension NSDictionary: WrapCustomizable {
    public func wrap(context: Any?, dateFormatter: DateFormatter?) -> Any? {
      return try? Wrapper(context: context, dateFormatter: dateFormatter)
          .wrap(dictionary: self as [NSObject : AnyObject])
    }
  }
#endif
      ...
      if let displayStyle = mirror.displayStyle {
        ...
        case .enum:
          if let wrappableEnum = value as? WrappableEnum {
            if let wrapped = wrappableEnum.wrap(context: self.context,
    ...
    for mirror in mirrors {
      for property in mirror.children {
        ...
        if let wrappingKey = wrappingKey {
          if let wrappedProperty = try customizable?
              .wrap(propertyNamed: propertyName,
                    originalValue: property.value,
                    context: self.context,
                    dateFormatter: self.dateFormatter) {
            wrappedDictionary[wrappingKey] = wrappedProperty
          } else {
            wrappedDictionary[wrappingKey] = try self.wrap(value: property.value,
