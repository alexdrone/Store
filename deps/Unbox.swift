...
extension Array: UnboxableCollection {
  ...
  public static func unbox<T: UnboxCollectionElementTransformer>(
      value: Any,
      allowInvalidElements: Bool,
      transformer: T) throws -> Array? where T.UnboxedElement == UnboxValue {
    ...
    return try array.enumerated().map(allowInvalidElements: allowInvalidElements) {
      index, element in
      let unboxedElement = try transformer.unbox(
          element: element,
...
extension Bool: UnboxableRawType {
 ...
 extension Data {
  ...
  func unbox<T: Unboxable>(allowInvalidElements: Bool) throws -> [T] {
    let array: [UnboxableDictionary] = try JSONSerialization.unbox(data: self,
  ...
  func unbox<T: UnboxableWithContext>(context: T.UnboxContext,
...
extension Dictionary: UnboxableCollection {
  ...
  public static func unbox<T: UnboxCollectionElementTransformer>(
      value: Any,
      allowInvalidElements: Bool,
      transformer: T) throws -> Dictionary? where T.UnboxedElement == UnboxValue {
    ...
    return try dictionary.map(allowInvalidElements: allowInvalidElements) { key, value in
      ...
      guard let unboxedValue = try transformer.unbox(
          element: value,
...
extension Int64: UnboxableRawType {
 ...
 extension JSONSerialization {
  static func unbox<T>(data: Data, options: ReadingOptions = []) throws -> T {
    do {
      return
        try (self.jsonObject(with: data, options: options) as? T).orThrow(UnboxError.invalidData)
    } catch {
...
extension Set: UnboxableCollection {
  ...
  public static func unbox<T: UnboxCollectionElementTransformer>(
      value: Any,
      allowInvalidElements: Bool,
      transformer: T) throws -> Set? where T.UnboxedElement == UnboxValue {
    guard let array = try [UnboxValue].unbox(value: value,
...
public typealias UnboxableDictionary = [String : Any]
 ...
 typealias UnboxTransform<T> = (Any) throws -> T?
...
public func unbox<T: Unboxable>(dictionary: UnboxableDictionary,
...
/// Throws `UnboxError`.
public func unbox<T: Unboxable>(dictionaries: [UnboxableDictionary],
...
/// Unbox an array JSON dictionary into an array of model `T` beginning at a certain key, 
/// optionally allowing invalid elements. Throws `UnboxError`.
public func unbox<T: Unboxable>(dictionary: UnboxableDictionary,
                                atKey key: String,
                                allowInvalidElements: Bool = false) throws -> [T] {
  let container: UnboxArrayContainer<T> = try unbox(dictionary: dictionary,
                                                    context: (.key(key), allowInvalidElements))
  return container.models
}

/// Unbox an array JSON dictionary into an array of model `T` beginning at a certain key path, 
/// optionally allowing invalid elements. Throws `UnboxError`.
public func unbox<T: Unboxable>(dictionary: UnboxableDictionary,
                                atKeyPath keyPath: String,
                                allowInvalidElements: Bool = false) throws -> [T] {
  let container: UnboxArrayContainer<T> =
      try unbox(dictionary: dictionary, context: (.keyPath(keyPath), allowInvalidElements))
  return container.models
}
...
/// Unbox binary data into an array of `T`, optionally allowing invalid elements.
public func unbox<T: Unboxable>(data: Data,
                                atKeyPath keyPath: String? = nil,
                                allowInvalidElements: Bool = false) throws -> [T] {
  if let keyPath = keyPath {
    return try unbox(dictionary: JSONSerialization.unbox(data: data),
                     atKeyPath: keyPath,
                     allowInvalidElements: allowInvalidElements)
  }
...
/// Unbox a JSON dictionary into a model `T` using a required contextual object.
public func unbox<T: UnboxableWithContext>(dictionary: UnboxableDictionary,
                                           context: T.UnboxContext) throws -> T {
  return try Unboxer(dictionary: dictionary).performUnboxing(context: context)
}

/// Unbox an array of JSON dictionaries into an array of `T` using a required contextual object,
/// optionally allowing invalid elements. Throws `UnboxError`.
public func unbox<T: UnboxableWithContext>(dictionaries: [UnboxableDictionary],
                                           context: T.UnboxContext,
                                           allowInvalidElements: Bool = false) throws -> [T] {
...
/// Unbox binary data into an array of `T` using a required contextual object, 
/// optionally allowing invalid elements. Throws `UnboxError`.
public func unbox<T: UnboxableWithContext>(data: Data,
                                           context: T.UnboxContext,
                                           allowInvalidElements: Bool = false) throws -> [T] {
...
/// Unbox `UnboxableDictionary` into a dictionary of type `[String: T]` where `T` is `Unboxable`.
public func unbox<T: Unboxable>(dictionary: UnboxableDictionary) throws -> [String: T] {
 ...
 struct UnboxArrayContainer<T: Unboxable>: UnboxableWithContext {
  let models: [T]
...
/// Protocol used to unbox an element in a collection.
/// Unbox provides default implementations of this protocol.
public protocol UnboxCollectionElementTransformer {
  ...
  /// Unbox an element from a collection, optionally allowing invalid elements for 
  /// nested collections
  func unbox(element: UnboxRawElement,
             allowInvalidCollectionElements: Bool) throws -> UnboxedElement?
}
...
// MARK: -  extensions
 ...
 struct UnboxContainer<T: Unboxable>: UnboxableWithContext {
  let model: T
...
public enum UnboxError: Error {
  ...
  /// An error occurred while unboxing a value for a path (contains the underlying 
...
// MARK: -  extensions
 ...
 extension UnboxFormatter {
  ...
  func makeCollectionTransform<C: UnboxableCollection>(
      allowInvalidElements: Bool) -> UnboxTransform<C> where C.UnboxValue == UnboxFormattedType {
    return {
      let transformer = UnboxFormatterCollectionElementTransformer(formatter: self)
      return try C.unbox(value: $0,
                         allowInvalidElements: allowInvalidElements,
...
private class UnboxFormatterCollectionElementTransformer<T: UnboxFormatter>:
    UnboxCollectionElementTransformer {
  ...
  func unbox(element: T.UnboxRawValue,
             allowInvalidCollectionElements: Bool) throws -> T.UnboxFormattedType? {
...
/// Protocol used to enable collections to be unboxed. 
...
public protocol UnboxableCollection: Collection, UnboxCompatible {
  ...
  static func unbox<T: UnboxCollectionElementTransformer>(
      value: Any,
      allowInvalidElements: Bool,
      transformer: T) throws -> Self? where T.UnboxedElement == UnboxValue
}
...
public extension UnboxableCollection {
  public static func unbox(value: Any, allowInvalidCollectionElements: Bool) throws -> Self? {
    ...
    if let unboxableType = UnboxValue.self as? Unboxable.Type {
      let transformer = UnboxCollectionElementClosureTransformer<UnboxableDictionary, UnboxValue>(){
        element in
        let unboxer = Unboxer(dictionary: element)
        return try unboxableType.init(unboxer: unboxer) as? UnboxValue
      }

      return try self.unbox(value: value,
                            allowInvalidElements: allowInvalidCollectionElements,
                            transformer: transformer)
    }

    if let unboxCompatibleType = UnboxValue.self as? UnboxCompatible.Type {
      let transformer = UnboxCollectionElementClosureTransformer<Any, UnboxValue>() {
        element in
        return try unboxCompatibleType.unbox(
            value: element,
            allowInvalidCollectionElements: allowInvalidCollectionElements) as? UnboxValue
      }
...
/// Protocol used to declare a model as being Unboxable with a certain context, for use with 
/// the unbox(context:) function
...
// MARK: -  extensions
 ...
 extension UnboxableWithContext {
  ...
  static func makeCollectionTransform<C: UnboxableCollection>(
      context: UnboxContext,
      allowInvalidElements: Bool) -> UnboxTransform<C> where C.UnboxValue == Self {
    return {
...
private class UnboxableWithContextCollectionElementTransformer<T: UnboxableWithContext>:
    UnboxCollectionElementTransformer {
 ...
 *
 *  For each supported type, simply call `unbox(key: string)` (where `string` is a key 
 *  in the dictionary that is being unboxed)
 *  - and the correct type will be returned. If a required (non-optional) value couldn't be 
 *  unboxed `UnboxError` will be thrown.
 */
public final class Unboxer {
  ...
  /// Initialize an instance with binary data than can then be decoded using the `unbox()` methods. 
  /// Throws `UnboxError` for invalid data.
  ...
  /// Perform custom unboxing using an Unboxer (created from a dictionary) passed to a closure,
  /// or throw an UnboxError
  public static func performCustomUnboxing<T>(dictionary: UnboxableDictionary,
                                              closure: (Unboxer) throws -> T?) throws -> T {
    return try Unboxer(dictionary: dictionary).performCustomUnboxing(closure: closure)
  }

  /// Perform custom unboxing on an array of dictionaries, executing a closure with a new Unboxer
  /// for each one, or throw an UnboxError
  public static func performCustomUnboxing<T>(array: [UnboxableDictionary],
  ...
  /// Perform custom unboxing using an Unboxer (created from binary data) passed to a closure,
  /// or throw an UnboxError
  public static func performCustomUnboxing<T>(
      data: Data,
      closure: @escaping (Unboxer) throws -> T?) throws -> T {
  ...
  public func unbox<C: UnboxableCollection, V: UnboxableWithContext>(
      key: String,
      context: V.UnboxContext,
      allowInvalidElements: Bool = false) throws -> C where C.UnboxValue == V {
    return try self.unbox(path: .key(key), transform: V.makeCollectionTransform(
        context: context,
        allowInvalidElements: allowInvalidElements))
  }
  ...
  public func unbox<C: UnboxableCollection, F: UnboxFormatter>(
      key: String,
      formatter: F,
      allowInvalidElements: Bool = false) throws -> C where C.UnboxValue == F.UnboxFormattedType {
  ...
  public func unbox<T: UnboxCompatible>(keyPath: String,
  ...
  public func unbox<F: UnboxFormatter>(keyPath: String,
                                       formatter: F) throws -> F.UnboxFormattedType {
  ...
  public func unbox<C: UnboxableCollection, V: UnboxableWithContext>(
      key: String,
      context: V.UnboxContext,
      allowInvalidElements: Bool = false) -> C? where C.UnboxValue == V {
  ...
  public func unbox<C: UnboxableCollection, F: UnboxFormatter>(
      key: String,
      formatter: F,
      allowInvalidElements: Bool = false) -> C? where C.UnboxValue == F.UnboxFormattedType {
    return try? self.unbox(key: key,
                           formatter: formatter,
                           allowInvalidElements: allowInvalidElements)
  }
